...
if __name__ == "__main__":
    ...
    for proc in proc_counts:
        for trial in range(5):
            filename = "../results/nonblock_throughput_%s_%d_%d.out" % (style, proc, trial + 1)
            try:
            ...
            filename = "../results/mutex_throughput_%s_%d_%d.out" % (style, proc, trial + 1)
            try:
                with open(filename) as outfile:
                    for line in outfile:
                        exec_time = float(line.split()[2])
    ...
    axes.grid(True)
    plot_path = "comp_throughput_%s.pdf" % style
